...
for i in <%= @hadoop_dir %>/*.jar; do CLASSPATH=$CLASSPATH:$i; done
CLASSPATH=<%= @hadoop_conf_dir %>:$CLASSPATH:<%= @hdfs_dir %>/hadoop-hdfs.jar
pushd .
cd $HADOOP_LIB_DIR
...
popd
su -s /bin/bash <%= @root_user %> -c "<%= @groovy_bin_dir %>/groovy -classpath $CLASSPATH <%= @init_hcfs_groovy %> <%= @hadoop_script_dir %>/init-hcfs.json"
