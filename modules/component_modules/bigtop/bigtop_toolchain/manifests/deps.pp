...
class bigtop_toolchain::deps inherits bigtop_toolchain::params
{

  $apache_prefix = nearest_apache_mirror()
  include bigtop_toolchain::packages
  exec {"/usr/bin/wget $apache_prefix/ant/binaries/${ant_bin_tar}":
    cwd     => "/usr/src",
    require => Package[$packages::pkgs],
    unless  => "/usr/bin/test -f /usr/src/${ant_bin_tar}",
  }
